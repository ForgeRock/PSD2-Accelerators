...
get_authorization_code_prompt_consent() {
    ...
    ac_response=$(curl\
        ...
        -v \
        "${openig_endpoint}/oauth2/authorize?response_type=code&scope=${scope}&client_id=${client_id}&redirect_uri=${redirect_uri}&prompt=${prompt}&claims=${_urlenc_claims}" 2>&1 | grep '< Location' | sed -e 's/< Location: //')
    echo "${ac_response}"
}
...
get_authorization_code_prompt_consent_url() {
    local prompt="consent";
    local _urlenc_claims=$(urlencode "${claims}")
    local _goto="${openig_endpoint}/oauth2/authorize?response_type=code&scope=${scope}&client_id=${client_id}&redirect_uri=${redirect_uri}&prompt=${prompt}&claims=${_urlenc_claims}"
    echo "${_goto}"
}
...
get_tokeninfo() {
    ...
    tokeninfo_response=$(curl\
        ...
        ${openig_endpoint}/oauth2/tokeninfo 2>&1)
...
get_userinfo() {
    ...
    userinfo_response=$(curl\
        ...
        -H "Cookie: ${cookie_name}=${sessionid}" \
        -H "Authorization: Bearer ${_access_token}" \
        ${openig_endpoint}/oauth2/userinfo 2>&1)
