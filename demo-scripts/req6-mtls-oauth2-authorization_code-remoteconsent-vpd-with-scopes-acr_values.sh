...
scope="uid%20openid%20https://www.yes.com/scopes/verified_person_data"
# ssl location
ssl_dir="ssl/"
# curl settings
curl_opts="-k"
# acr_values
acr_values="https://www.no.com/acrs/online_banking_sca"
...
get_authorization_code_prompt_consent() {
    ...
    ac_response=$(curl\
        ...
        -v \
        "${openam_endpoint}/oauth2/authorize?response_type=code&acr_values=${acr_values}&scope=${scope}&client_id=${client_id}&redirect_uri=${redirect_uri}&prompt=${prompt}" 2>&1 | grep '< Location' | sed -e 's/< Location: //')
    echo "${ac_response}"
}
