...
# get an authorization code with an id_token_hint and session
get_authorization_code_ith_session() {
    local sessionid="${1}";
    local idtoken="${2}";
    ac_response=$(curl\
        -s \
        -X POST ${curl_opts} \
        -H "Cookie: ${cookie_name}=${sessionid}" \
        -d "response_type=code&scope=${scope}&client_id=${client_id}&nonce=123&csrf=${sessionid}&redirect_uri=${redirect_uri}&decision=allow&id_token_hint=${idtoken}" \
        -H 'Content-Type: application/x-www-form-urlencoded' \
        -v \
        ${openam_endpoint}/oauth2/authorize 2>&1 | grep '< Location' | sed -e 's/< Location: //')
    ac_code=$(echo ${ac_response} | awk -F'?' '{ print $2 }'| awk -F'&' '{ print $1 }' | awk -F'=' '{ print $2 }')
...
_id_token_hint_response_session=$(get_authorization_code_ith_session "${sessionid}" "123${id}")
echo "Authorize call with id_token_hint and session response: "
echo "${_id_token_hint_response_session}"
