...
# client certificate
client_cert=${ssl_dir}different_client.crt
...
# perform mtls request with header for a different client towards introspection endpoint
get_introspect_differentclient() {
    local _access_token="${1}";
    local _token_resp=$(curl \
        -s \
        -X POST ${curl_opts} \
        -H "${mtls_header}: ${certificate_header}" \
        -d "client_id=${client_id2}&token_type_hint=access_token&token=${_access_token}" \
...
_introspect_dc_response=$(get_introspect_differentclient "${at}")
echo "Response from introspect endpoint as a different client: "
jq . <<< ${_introspect_dc_response}
echo
