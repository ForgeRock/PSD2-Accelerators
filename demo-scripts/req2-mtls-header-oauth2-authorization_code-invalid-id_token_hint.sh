...
# get an authorization code with an id_token_hint
get_authorization_code_ith() {
    local idtoken="${1}";
    ac_response=$(curl\
        -s \
        -X POST ${curl_opts} \
...
_id_token_hint_response=$(get_authorization_code_ith "123${id}");
echo "Authorize call with invalid id_token_hint response: "
echo "${_id_token_hint_response}"
