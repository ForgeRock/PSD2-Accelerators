...
public class JWTManagementService {	
	...
	public RSAKey senderKey() throws JOSEException, NoSuchAlgorithmException, IOException, ParseException {
		if(senderJWK==null) {
			...
			/*
			File file =  ResourceUtils.getFile("classpath:config/jwks/jwk-set.json");
			log.debug("File Found :{} ",file.exists());
			log.debug("File content: {} ",new String(Files.readAllBytes(file.toPath())));
			
			JWKSet localKeys = JWKSet.load(file);
			List<JWK> jwkSets= localKeys.getKeys();
			
			senderJWK  = RSAKey.parse(localKeys.toJSONObject(false));
			*/
		}
...
//    }
	...
	/*
	public String senderSingEncripyJWT(JWTClaimsSet jwtClaimsSet, ApplicationProperties amConfiguration) throws JOSEException, MalformedURLException, IOException, ParseException {
		JWKSet publicKeys = JWKSet.load(new URL(amConfiguration.getAmJwkUrl()));
		log.debug("publicKeys {}",publicKeys.toString());	
		
		//TODO Get Encryption algorithm, KID and signature  
		// Create JWT
		SignedJWT signedJWT = new SignedJWT(
				new JWSHeader.Builder(JWSAlgorithm.RS256).keyID(senderKey().getKeyID()).build(),
				 jwtClaimsSet);
		...
		jweObject.encrypt(new RSAEncrypter(recipientKey().toPublicJWK()));
