...
import com.forgerock.openbanking.aspsp.rs.rcs.model.consent.obpcr.Account;
...
public class ConsentViewController {
	...
	public String psd2ASPSPConsent(
		...
		try {
			...
			model.addAttribute(CLIENT_NAME, parsedSet.getClaims().get(CLIENT_NAME));
			model.addAttribute(STATE, parsedSet.getClaims().get(STATE));
			model.addAttribute(USERNAME, parsedSet.getClaims().get(USERNAME));
			...
			model.addAttribute(CLAIMS, fronClaimsKey);
				...
				try {
					...
					if (!StringUtils.isEmpty(consentID)) {
						...
						if (OpenBankingConstants.PISP.PISP_FLOW.equals(reqestFlow)) {
							...
							idmRequestBody = consentManagement.buildPispBody(sub, claims);
							...
							idmURL = new StringBuilder().append(applicationProperties.getIdmUpdateAccountConsentUrl())
									.append(consentID).append("?_action=patch");
							idmRequestBody = consentManagement.buildAispBody(sub, claims, accounts);
						}
